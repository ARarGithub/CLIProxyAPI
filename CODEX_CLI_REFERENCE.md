...
| `x-oai-attestation` | All paths（attestation provider 啟用時） | 設備驗證簽章字串 | `attestation_provider.header_for_request().await` | per request；需要 attestation 配置 | `core/src/client.rs:501-503`+`908-910` | ✅ H 完成（**strip**：cross-auth correlation 風險 + pool 下 binding 失敗。Real client 沒設 attestation provider 也不送，strip 後 fork 看起來像「沒設」的合法配置） |
...
**D. 修 L1 patch 拆兩個 derive stream**（最高優先）— ✅ **已完成 2026-05-11**
- 新增兩個 helper：`derivedSessionID` 跟 `derivedThreadID`（namespace 不同 → 對任何 (originalID, auth.ID) 都保證輸出不同）
- header 寫入：
  - `Session_id` ← `sessionDerived`
  - `Thread_id` / `X-Client-Request-Id` ← `threadDerived`（**完全相同字串**，mirror real client）
- body 寫入：
  - `prompt_cache_key` ← `threadDerived`（real client 用 thread_id 字串，不是 session_id）
- 詳見 `LEAK_RISKS.md` L1、`SESSION_ID_BEHAVIOR.md`、`MERGE_GUIDE.md` L1 不變式表

**E. 移除 `Conversation_id` header** — ✅ **已完成 2026-05-11**
- `applyCodexPromptCacheHeaders` 內的 `headers.Set("Conversation_id", cache.ID)` 已刪除
...
- 在 hardening hook 內讀 inbound gin headers 的 `X-Openai-Subagent` / `X-Codex-Parent-Thread-Id`，沒有就不送（real client 也不送）
- `X-Openai-Subagent`：verbatim passthrough（subagent label like `"review"` / `"compact"`，本身無 cross-auth correlation 風險）
- `X-Codex-Parent-Thread-Id`：per-auth derive 經 `derivedThreadID(parent, auth)`，避免 pooled auths 同一 parent 被關聯
- `X-Oai-Attestation`：**永遠 strip**（hardening 主動刪除任何 case variant）。原因：opaque token 簽自 inbound 客戶端的 OAuth，pool 下 outbound 配的是不同 OAuth bearer → 兩次 dispatch 同一 inbound 送出**相同** attestation 配兩個 bearer = cross-auth 強關聯；且 attestation binding 對不上會被上游 reject。Real client 沒配 attestation provider 也不送這條，strip 後 fork 看起來像「沒配」的合法配置。
...
| 2026-05-11 | 完成 §6 H / I / J。新增 conditional codex CLI headers 處理：`X-Openai-Subagent` verbatim passthrough（純 label，無 cross-auth 風險）；`X-Codex-Parent-Thread-Id` per-auth derive（避免 pooled auths 同 parent 被關聯）。WS path 用 lowercase 變體 + WS body 的 `client_metadata.x-codex-parent-thread-id` 也 derive、其它 `client_metadata` 條件欄位（subagent / turn_metadata）passthrough。Regression test 新增 `codex_direct_with_subagent_flow` scenario + per-record conditional header invariant assertion（inbound 有→outbound 有 + parent 必 derive；inbound 無→outbound 必無）+ crossAuthLeakFields 加 `X-Codex-Parent-Thread-Id` 與 `client_metadata.x-codex-parent-thread-id`。+8 條測試（HTTP 3、WS 5），共 50 條 fingerprint 相關測試全綠。 | main HEAD |
| 2026-05-11 | **Audit 修正**：`X-Oai-Attestation` 從「verbatim passthrough」改為「永遠 strip」。原本實作會在池化下產生 cross-auth identical attestation token = pool signal；改 strip 後 fork 看起來像「沒配 attestation provider」的合法 real client。Regression test 對所有 scenario 加 `X-Oai-Attestation` 必為空斷言；新單元測試 `TestCodexFingerprintHardeningHTTP_Attestation_StrippedEvenWhenInboundPresent`；`X-Openai-Subagent` 加進 `genericNoiseFieldsAllowedIdentical`（label 是 fixed enum，cross-auth 同值不關聯帳號）。Code audit pass：regression test 跑 verbose `-v` 後 zero「potential new leak」warning。共 52 條 fingerprint 相關測試全綠。 | main HEAD |
