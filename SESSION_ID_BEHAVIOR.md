...
UUIDv5 = `uuid.NewSHA1(namespace, name)`，純 SHA-1 hash，第 14 位是 `5`。
UUIDv7（real Codex CLI 用的）= 48-bit unix-ms timestamp + version=7 + 12-bit rand + variant + 62-bit rand，第 14 位是 `7`。

**OpenAI 只需要看第 14 位就能分辨**：v7 = 真實 client，其他 = 不對。Decode v7 timestamp（前 48 bit）的成本同樣極低（單一算術操作），所以「v7 但 timestamp 是 1972 年」也會立刻被抓出來。L1 patch 因此**同時偽造 v7 結構 + 注入合理 timestamp**，純 SHA1 derive 不夠。
